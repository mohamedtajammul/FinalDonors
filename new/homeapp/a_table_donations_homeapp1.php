...
$sql = "SELECT * FROM homeappliance where status=0 ORDER BY date DESC";
...
<body style="background-color:#515151;">
    ...
    <h1 style="font-family:Montserrat, sans-serif;color:#ffa500;">HOME APPLIANCES DONATION</h1>
	<h5 style="font-family:Montserrat, sans-serif;color:#ffa500;">OLD DONATIONS</h5>
    <div id="div1"><button class="btn btn-primary" type="button" data-bs-hover-animate="pulse" style="font-family:Montserrat, sans-serif;" onclick="location.href='http:a_table_donations_homeapp.php'">VIEW NEW DONATIONS</button></div>
    ...
    while($row = mysqli_fetch_assoc($result)) {
		$ha_id=$row["ha_id"]; $name=$row["name"]; $address=$row["address"];$date=$row["date"];
     echo "<tr style='font-family:Montserrat, sans-serif;'>";
    echo "<td>" . $name ."</td>";
    echo "<td>" . $address . "</td>";
    echo "<td>" . $date . "</td>";
	echo "<td><button class='btn btn-primary' type='button' data-bs-hover-animate='pulse' onclick=\"window.open('http:hdonation_view.php?link=$ha_id','_blank')\">view</button></td>";
    echo " <td><button class='btn btn-primary' type='button' data-bs-hover-animate='pulse' onclick=\"location.href='http:hdelete.php?link=$ha_id&set=0'\">delete</button></td>";
	echo "</tr>";
    }
} else {
    echo "No Old Donations";
}
